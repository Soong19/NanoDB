...
public class SqlTestCase {
    ...
    public void beforeClass() throws Exception {
        ...
        if (!testBaseDir.exists()) {
            if (!testBaseDir.mkdirs())
                throw new IOException("Couldn't create test base directory");
        }
        else {
        ...
        props.setProperty(ServerProperties.PROP_BASE_DIRECTORY,
            testBaseDir.getAbsolutePath());

        server.startup(props);
    ...
    public void checkResultSchema(CommandResult result, String... expected) {
        ...
        for (int i = 0; i < schema.numColumns(); i++) {
            ...
            assert Objects.equals(colInfo.getTableName(), tblName) :
                "Expected column " + i + " to have table-name " + tblName;
            assert Objects.equals(colInfo.getName(), colName) :
     ...
     */
    public void testTableNotEmpty(String tableName) {
        CommandResult result =
            server.doCommand("SELECT * FROM " + tableName, true);
