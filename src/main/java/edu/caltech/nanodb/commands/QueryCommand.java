...
public abstract class QueryCommand extends Command {
    ...
    public void execute(NanoDBServer server) {
        ...
        if (!explain) {
            ...
            if (cost != null) {
                logger.debug(String.format("Estimated %f tuples with average size %f bytes",
            ...
            TupleProcessor processor = getTupleProcessor(server.getEventDispatcher());

            EvalStats stats;
            try {
                stats = QueryEvaluator.executePlan(plan, processor);
            } catch (Exception e) {
            ...
            PlanCost cost = plan.getCost();
            if (cost != null) {
                out.printf("Estimated %f tuples with average size %f%n", cost.numTuples, cost.tupleSize);
                out.println("Estimated number of block IOs:  " + cost.numBlockIOs);
                logger.debug("Estimated CPU cost:  " + cost.cpuCost);
            } else {
